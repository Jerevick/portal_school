...
- Create two apps on Fly, one for staging and one for production:
  ...
  ```sh
  fly apps create portal_school-ff62
  fly apps create portal_school-ff62-staging
  ```
...
- Add a `SESSION_SECRET` to your fly app secrets, to do this you can run the following commands:
  ...
  ```sh
  fly secrets set SESSION_SECRET=$(openssl rand -hex 32) --app portal_school-ff62
  fly secrets set SESSION_SECRET=$(openssl rand -hex 32) --app portal_school-ff62-staging
  ```

  > **Note:** When creating the staging secret, you may get a warning from the Fly CLI that looks like this:
  >
  > ```
  > WARN app flag 'portal_school-ff62-staging' does not match app name in config file 'portal_school-ff62'
  > ```
  >
...
- Create a database for both your staging and production environments. Run the following:
  ...
  ```sh
  fly postgres create --name portal_school-ff62-db
  fly postgres attach --app portal_school-ff62 portal_school-ff62-db

  fly postgres create --name portal_school-ff62-staging-db
  fly postgres attach --app portal_school-ff62-staging portal_school-ff62-staging-db
  ```
